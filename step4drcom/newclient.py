...
# CONFIG
server = '10.100.61.3'
username='jiafeng17'#用户�?
password='aa123123'#密码
host_ip = '49.140.7.29'#ip地址
mac = 0x000C43762078#mac地址
host_name = 'PandoraBox'#计算机名
host_os = 'Linux'#操作系统
...
def mkpkt(salt, usr, pwd, mac):
    ...
    data += dump(mac)
    if (len(pwd) / 4) != 4:
        data += '\x00' * (len(pwd) / 4)#strange。。�?    data += '\x60\xa2' #unknown, filled numbers randomly =w=
    data += '\x00' * 28
